...
namespace tjc.Modules.jacs.Handlers
{
    ...
    public class DocketHandler : IHttpHandler
    {
                        ...
                        foreach (var kvp in sortedHearings)
                        {
                            ...
                            if (categoryPrint && categories.Count > 1)
                            {
                                foreach (var cat in categories.OrderBy(c => c))
                                {
                                    ...
                                    AddHearingsTable(body, catHearings);
                                }
                            }
                            else
                            {
                                AddHearingsTable(body, dayHearings);
                            }
        ...
        private void AddHearingsTable(Body body, List<Dictionary<string, object>> hearings)
        {
            ...
            foreach (var h in hearings)
            {
                ...
                else
                {
                    ...
                    if (!string.IsNullOrEmpty(template))
                    {
                        ...
                        var udfs = json.Properties().Where(p => !string.IsNullOrEmpty(p.Name)).Select(p => new { Label = CleanLabel(p.Name), Value = p.Value.ToString() }).ToList();
                        for (int i = 0; i < udfs.Count; i += 3)
                        {
                            ...
                            for (int j = 0; j < 3; j++)
                            {
                                ...
                                {
                                    var udf = udfs[i + j];
                                    udfRow.Append(new TableCell(CreateLabeledParagraph(udf.Label, udf.Value, JustificationValues.Left)));
                                }
                                else
                                {
                                    udfRow.Append(new TableCell(CreateParagraph("")));
                                }
                            }
