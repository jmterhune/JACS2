        ...
        private void AddHearingsTable(Body body, List<Dictionary<string, object>> hearings, long courtId)
        {
            ...
            foreach (var h in hearings)
            {
                ...
                else
                {
                    ...
                    // Row 3
                    string attorney = h["attorney"].ToString().Trim();
                    string attorneyLastFirst = "Unknown";

                    if (!string.IsNullOrWhiteSpace(attorney) && attorney != "Unknown")
                    {
                        var parts = attorney.Split(new[] { ' ' }, StringSplitOptions.RemoveEmptyEntries);
                        if (parts.Length >= 2)
                        {
                            // Common suffixes (case-insensitive)
                            var commonSuffixes = new[] { "jr", "jr.", "sr", "sr.", "ii", "iii", "iv", "v", "vi", "esq", "esq." };
                            ...
                            if (commonSuffixes.Contains(lastPart.ToLowerInvariant()))
                            {
                                suffix = lastPart;
                                lastName = parts.Length >= 3 ? parts[parts.Length - 2] : parts[0];
                            }

                            // Build the name: Last, First [Middle(s)] [Suffix]
                            var firstAndMiddle = parts.Take(parts.Length - (suffix.Length > 0 ? 2 : 1)).ToList();
                            string firstPart = firstAndMiddle.FirstOrDefault() ?? "";
                            string middleParts = string.Join(" ", firstAndMiddle.Skip(1));

                            attorneyLastFirst = lastName;

                            if (!string.IsNullOrEmpty(firstPart))
                            {
                                attorneyLastFirst += ", " + firstPart;
                                if (!string.IsNullOrEmpty(middleParts))
                                    attorneyLastFirst += " " + middleParts;
                            }

                            if (!string.IsNullOrEmpty(suffix))
                                attorneyLastFirst += " " + suffix;
                        }
                        else
                        {
                    ...
                    string oppAttorney = h["opp_attorney"].ToString().Trim();
                    string oppLastFirst = "Unknown";

                    if (!string.IsNullOrWhiteSpace(oppAttorney) && oppAttorney != "Unknown")
                    {
                        var parts = oppAttorney.Split(new[] { ' ' }, StringSplitOptions.RemoveEmptyEntries);
                        if (parts.Length >= 2)
                        {
                            var commonSuffixes = new[] { "jr", "jr.", "sr", "sr.", "ii", "iii", "iv", "v", "vi", "esq", "esq." };
                            ...
                            oppLastFirst = lastName;

                            if (!string.IsNullOrEmpty(firstPart))
                            {
                                oppLastFirst += ", " + firstPart;
                                if (!string.IsNullOrEmpty(middleParts))
                                    oppLastFirst += " " + middleParts;
                            }
