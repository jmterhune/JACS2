...
namespace tjc.Modules.PretrialServices
{
    ...
    /// 
    /// Because the control inherits from PretrialServicesSettingsBase you have access to any custom properties
        ...
        public override void LoadSettings()
        {
            try
            {
                if (Page.IsPostBack == false)
                {
                    ...
                    drpCounty.DataBind();
                    if (Settings.Contains("CountyId"))
                        drpCounty.SelectedValue = Settings["CountyId"].ToString();
                    if(Settings.Contains("ReportUrl"))
                        txtReportUrl.Text= Settings["ReportUrl"].ToString();
                }
            }
