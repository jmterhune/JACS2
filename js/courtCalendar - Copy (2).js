...
class CourtCalendarController {
    ...
    populateMotionSelectExcludingRestricted() {
        ...
        $.ajax({
            url: `${this.service.baseUrl}CourtMotionAPI/GetAvailableMotionsDropDown/${this.courtId}?excludedIds=${restrictedIds.join(',')}`,
            type: 'GET',
            dataType: 'json',
            beforeSend: xhr => this.setAjaxHeaders(xhr),
            success: response => {
                ...
                if (select && response.data) {
                    response.data.push({ Key: 0, Value: 'Other' });
                    select.innerHTML = '';
                    response.data.forEach(item => {
                        const option = document.createElement('option');
                        option.value = item.Key;
            ...
            success: response => {
                ...
                selects.forEach(id => {
                    ...
                    if (select && response.data) {
                        ...
                        select.setAttribute('autocomplete', 'off');
                        if (this.courtData) {
                            if (id === 'event_attorney') {
                                if (this.courtData.def_attorney_id) {
                                    ts.setValue(this.courtData.def_attorney_id.toString());
                                }
                            } else if (id === 'event_opposingAttorney') {
                                if (this.courtData.opp_attorney_id) {
    ...
    populateCaseNumFields() {
        ...
        if (this.courtData && this.courtData.case_num_format) {
            ...
            parts.forEach((part, index) => {
                const input = $(`<input type="text" class="form-control case-num-part mr-1" placeholder="${part}">`);
            ...
            container.append('<input type="text" id="event_caseNum" class="form-control" autocomplete="off">');
        }
    }
    ...
    bindEventHandlers() {
        ...
        $('#timeslot_allDay').on('change', (e) => {
            if (e.target.checked) {
                $('.time-selection').hide();
                $('.quantity-group').hide();
                $('.cattle-call').hide();
                $('#timeslot_duration').val('1440');
    ...
    onTimeslotModalShow() {
        ...
        $('#rescheduleBtn').hide();
        const allDayChecked = $('#timeslot_allDay').is(':checked');
        ...
        $('.nav-tabs a').on('shown.bs.tab', (e) => {
            if (e.target.hash === '#eventTab') {
                ...
                const motionVal = $('#event_motion').val();
                if (motionVal === '0') {
    ...
    getEventFormData() {
        ...
        return {
            id: evtId,
            case_num: caseNum,
            motion: motionTom ? motionTom.getValue() : '',
            type: typeTom ? typeTom.getValue() : '',
            otherMotion: $('#event_otherMotion').val(),
            attorney: attorneyTom ? attorneyTom.getValue() : '',
            opposingAttorney: opposingAttorneyTom ? opposingAttorneyTom.getValue() : '',
        ...
        $.ajax({
            ...
            success: response => {
                if (response.data) {
                    ...
                    const motionTom = $('#event_motion')[0].tomselect;
                    if (motionTom) motionTom.setValue(response.data.motion);
                    const typeTom = $('#event_type')[0].tomselect;
                    if (typeTom) typeTom.setValue(response.data.type);
                    $('#event_otherMotion').val(response.data.otherMotion);
                    const attorneyTom = $('#event_attorney')[0].tomselect;
                    if (attorneyTom) attorneyTom.setValue(response.data.attorney);
                    const oppAttorneyTom = $('#event_opposingAttorney')[0].tomselect;
                    if (oppAttorneyTom) oppAttorneyTom.setValue(response.data.opposingAttorney);
            ...
            success: response => {
                ...
                if (response.data) {
                    if (response.data.length === 1) {
                        ...
                        const motionTom = $('#event_motion')[0].tomselect;
                        if (motionTom) motionTom.setValue(e.motion);
                        const typeTom = $('#event_type')[0].tomselect;
                        if (typeTom) typeTom.setValue(e.type);
                        $('#event_otherMotion').val(e.otherMotion);
                        const attorneyTom = $('#event_attorney')[0].tomselect;
                        if (attorneyTom) attorneyTom.setValue(e.attorney);
                        const oppAttorneyTom = $('#event_opposingAttorney')[0].tomselect;
                        if (oppAttorneyTom) oppAttorneyTom.setValue(e.opposingAttorney);
