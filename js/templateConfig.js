...
class TemplateConfigController {
    ...
    bindEventHandlers() {
        ...
        $('#saveTimeslotBtn').on('click', this.handleSaveClick.bind(this));
                    ...
                    $.ajax({
                        ...
                        success: response => {
                            if (response.status === 200) {
                                this.calendar.refetchEvents();
                                bootstrap.Modal.getInstance(document.getElementById('TimeslotModal')).hide();
                                ShowNotification('Success', response.message || 'Timeslot deleted successfully.', 'success');
                            } else {
                                ShowNotification('Error', response.message || 'Failed to delete timeslot.', 'error');
                            }
                        },
    ...
    getFormData() {
        ...
        return {
            ...
            court_template_id: this.templateId,
    ...
    updateTimeslot(info) {
        ...
        $.ajax({
            ...
            success: response => {
                if (response.status === 200) {
                    this.calendar.refetchEvents();
                    ShowNotification('Success', response.message || 'Timeslot updated successfully.', 'success');
                } else {
                    ShowNotification('Error', response.message || 'Failed to update timeslot.', 'error');
                }
            },
