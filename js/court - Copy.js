...
class CourtController {
        ...
        $(selector).select2({
            ajax: {
                ...
                error: function () {
                    ...
                    ShowAlert("Error", "Failed to retrieve attorney records. Make sure you are logged in and try again.");
                },
                cache: true
            },
        ...
        $(selector).select2({
            ajax: {
                ...
                error: function () {
                    ...
                    ShowAlert("Error", "Failed to retrieve motion records. Make sure you are logged in and try again.");
                },
                cache: true
            },
            placeholder: placeholder,
        ...
        $(selector).select2({
            ajax: {
                ...
                error: function () {
                    ...
                    ShowAlert("Error", "Failed to retrieve hearing event type records. Make sure you are logged in and try again.");
                },
                cache: true
            },
            placeholder: placeholder,
    ...
    ViewCourt(courtId, isEditMode = false) {
        ...
        if (courtId) {
            $.ajax({
                ...
                error: function (error) {
                    if (error.status === 401) {
                        ShowAlert("Error Retrieving Court Details", "Please make sure you are logged in and try again. Error: " + error.statusText);
                    } else {
                        ShowAlert("Error Retrieving Courts Details", "The following error occurred attempting to retrieve court information. Error: " + error.statusText);
                    }
                    $(progressId).hide();
                }
            });
    ...
    CreateCourt() {
        try {
            ...
            $("#edit_progress-court").hide();
            ShowAlert("Error Creating Court", e.message);
        }
    }
    ...
    UpdateCourt() {
        try {
            ...
            $("#edit_progress-court").hide();
            ShowAlert("Error Updating Court", e.message);
        }
    }
