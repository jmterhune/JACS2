...
class CourtController {
    ...
    initEdit() {
        ...
        if (this.courtId) {
            this.ViewCourt(true);
            this.retrieveTemplateDropdownOptions();
        } else {
    ...
    retrieveTemplateDropdownOptions() {
        $.ajax({
    ...
    addTemplateRow(templateId = '', week = '', date = '', auto = null) {
        ...
        const $newSelect = $('#templates_table tbody tr:last-child select.template-select');
        $newSelect.empty().append('<option value="">-</option>');
        this.templateOptions.forEach(template => {
            $newSelect.append(`<option value="${template.Key}">${template.Value}</option>`);
        });
        $newSelect.select2({
