...
namespace tjc.Modules.ExpertWitness
{
    public partial class View : ExpertWitnessModuleBase
    {
        ...
        protected void cmdUpdate_Click(object sender, EventArgs e)
        {
            ...
            if (errorMessage == "")
            {
                ...
                if (errorMessage != "")
                {
                    ...
                    plhMessage.Controls.Add(Skin.GetModuleMessageControl("Request Failed!", errorMessage, ModuleMessage.ModuleMessageType.RedError));
                }
                else
                {
                    rtCtl.DeleteRequestTemplatesByGuid(GlobalID);
                    ...
                    plhMessage.Controls.Add(Skin.GetModuleMessageControl("Success!", "Your request has been added successfullty", ModuleMessage.ModuleMessageType.GreenSuccess));
                }
            }
            else
                plhMessage.Controls.Add(Skin.GetModuleMessageControl("Requirements not fullfilled", errorMessage, ModuleMessage.ModuleMessageType.YellowWarning));
        }
            ...
            if (e.CommandName == "add")
            {
                try
                {
                    if (AddExpert(seq))
                    {
                        ...
                        plhMessage.Controls.Add(Skin.GetModuleMessageControl("Add Expert Failed", "No more experts remain related to required specification", ModuleMessage.ModuleMessageType.YellowWarning));
                }
                catch (Exception exc)
                {
                    Exceptions.ProcessModuleLoadException(this, exc);
        ...
        private void GetExperts()
        {
            ...
            foreach (TemplateSequence tt in tCtl.GetTemplateSequences(templateId).OrderBy(o => o.Sequence))
            {
                ...
                tempRequestedTemplate = rCtl.GetTemporaryRequestedTemplates(tt.TemplateID, tt.Sequence, locationId)
                   .Where(x => !listSelectedExperts.Contains(x.ExpertID) && !listRequestedTemplate.Select(lt => lt.ExpertID).Contains(x.ExpertID))
                   .Select(r => new RequestedTemplate() { ExpertID = r.ExpertID, ExpertName = r.ExpertName, Comments = r.Comments, TemplateID = templateId, NumberRequired = r.NumberRequired, Header = r.Header, Sequence = tt.Sequence, Guid = GlobalID, Status = Convert.ToInt32(RequestStatus.unselected), Position = r.Position }).Distinct().OrderBy(x => x.Position).ToList().Except(PassedExperts, new RequestedTemplateComparer()).Take(numberRemaining).ToList();

                if (tempRequestedTemplate.Count < numberRemaining)
                    errorMessage += "<li>There are not enough experts available to fullfill requirement # " + tt.Sequence.ToString() + "</li>";
            ...
            if (errorMessage != "")
            {
                ...
                plhMessage.Controls.Add(Skin.GetModuleMessageControl("Template Requirements Were Not Met", errorMessage, ModuleMessage.ModuleMessageType.RedError));
            }
        ...
        private bool AddExpert(int sequence)
        {
            ...
            var ctl = new RequestedTemplateController();
            var listSelectedExperts = ctl.GetRequestedTemplatesByGuidByStatus(GlobalID, sequence).Select(i => i.ExpertID);
            IEnumerable<RequestedTemplate> requestedTemplates = ctl.GetRequestedTemplatesByTemplateByLocationBySequence(templateId, locationId, sequence).Where(x => !listSelectedExperts.Contains(x.ExpertID));
            RequestedTemplate requestedTemplate = (requestedTemplates.Select(x => new RequestedTemplate { ExpertID = x.ExpertID, ExpertName = x.ExpertName, Comments = x.Comments, TemplateID = templateId, Sequence = sequence, Guid = GlobalID, Status = Convert.ToInt32(RequestStatus.unselected) }).FirstOrDefault());
