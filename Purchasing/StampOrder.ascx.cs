            ...
            try
            {
                ...
                else
                {
                    System.Web.UI.ScriptManager.RegisterStartupScript(this, GetType(), "msg" + Guid.NewGuid().ToString("N"),
                        "Swal.fire({ title: 'Error', html: '" + System.Web.HttpUtility.JavaScriptStringEncode("Unable to Add Record. Please contact the <a href='mailto:helpdesk@jud12.flcourts.org'>help desk</a>.") + "', icon: 'error', confirmButtonText: 'OK' });", true);
                }
            }
            catch (Exception exc)
            {
