...
namespace tjc.Modules.JudicialReferral.Views
{
    public partial class Referral : JudicialReferralModuleBase
    {
        ...
        {
            // Force the case-type and defendant suffix textbox values to upper case
            // so the displayed and stored values match. PadSequence in GetCaseNumber
            // already pads the sequence to 6 chars.
            txtCaseType.Text = (txtCaseType.Text ?? string.Empty).Trim().ToUpper();
            txtDefendantSuffix.Text = (txtDefendantSuffix.Text ?? string.Empty).Trim().ToUpper();
        ...
        /// <summary>
        /// Strip to digits and left-pad to 6 (X-YYYY-XX-000001 convention).
        /// Returns empty string if no digits — validator blocks the save.
        /// </summary>
