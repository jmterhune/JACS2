...
(function (window, $) {
    ...
    /* ------------------------------------------------------------------
     *  Supervisor dropdown: wrap options in <optgroup> sections.
     *
     *  ASP.NET's DropDownList renders flat <option> elements regardless
     *  of how you arrange the ListItems server-side. The Supervisor
     *  dropdown comes out with each option tagged
     *  data-supgroup="active|inactive"; this hook walks the options,
     *  pulls consecutive same-group items into a real
     *  <optgroup label="Active|Inactive">, and removes the data attr
     *  on the way out. The optgroup labels render as bold section
     *  headers natively in every browser — that's the "Active /
     *  Inactive" header the user sees in the open dropdown.
     * ------------------------------------------------------------------ */
    function groupSupervisorDropdown() {
        // The ID is generated by ASP.NET — match by the trailing literal
        // (ClientIDMode for drpSupervisor isn't Static, so the rendered id
        // looks like "...$ctl00$drpSupervisor" -> "..._drpSupervisor").
        var selects = document.querySelectorAll('select[id$="drpSupervisor"]');
        for (var i = 0; i < selects.length; i++) {
            applySupervisorOptgroups(selects[i]);
        }
    }
    function applySupervisorOptgroups(sel) {
        // Collect the in-order list of options that carry data-supgroup;
        // anything without it (blank placeholder) is left in place.
        var options = sel.querySelectorAll('option[data-supgroup]');
        if (options.length === 0) return;

        var labels  = { active: "Active", inactive: "Inactive" };
        var current = null;     // currently-open <optgroup> element
        var currentName = null; // the data-supgroup value for `current`

        for (var i = 0; i < options.length; i++) {
            var opt = options[i];
            var grp = opt.getAttribute("data-supgroup");
            if (grp !== currentName) {
                current = document.createElement("optgroup");
                current.label = labels[grp] || grp;
                sel.appendChild(current);
                currentName = grp;
            }
            // Moving the option into the optgroup detaches it from its
            // current parent automatically — no removeChild needed.
            current.appendChild(opt);
            // Strip the marker attribute now that the group is in place.
            opt.removeAttribute("data-supgroup");
        }
    }
