...
{
    /// <summary>REST endpoints for the Classes admin tab on EmployeeList.</summary>
    [DnnModuleAuthorize(AccessLevel = SecurityAccessLevel.View)]
    [ValidateAntiForgeryToken]
    public class JobClassesController : DnnApiController
    {
        ...
        private readonly JobClassController _ctrl;

        public JobClassesController()
        {
            _ctrl = new JobClassController(_hostSettings);
        }
        ...
        {
            try { return Request.CreateResponse(HttpStatusCode.OK, _ctrl.GetAll().OrderBy(x => x.ClassName)); }
        ...
        [HttpPost]
        public HttpResponseMessage Post(JobClassInfo item)
        {
        ...
        [HttpPut]
        public HttpResponseMessage Put(int id, JobClassInfo item)
        {
            if (item == null) return Request.CreateErrorResponse(HttpStatusCode.BadRequest, "Body required");
            try
            {
