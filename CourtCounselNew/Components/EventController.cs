...
namespace tjc.Modules.CourtCounsel.Components
{
    internal class EventController
    {
        ...
        public async Task<string> CreateGraphEventAsync(Event t, string userName, int portalId)
        {
            try
            {
                ...
                GraphClient graphClient = new GraphClient(config.APIKey, config.APISecret, config.TenantId);
        ...
        public async Task<bool> DeleteGraphEventAsync(string graphEventId, string userName, int portalId)
        {
            try
            {
                // Initialize the GraphServiceClient with authentication details.
                var client = new Microsoft.Graph.GraphServiceClient(new Microsoft.Graph.DelegateAuthenticationProvider(async (requestMessage) =>
                {
                    // Add authentication headers here (e.g., Bearer token).
                    requestMessage.Headers.Authorization = new System.Net.Http.Headers.AuthenticationHeaderValue("Bearer", "YOUR_ACCESS_TOKEN");
                }));

                // Delete the event from the user's calendar.
                await client.Users[userName].Events[graphEventId].Request().DeleteAsync();
                return true;
            }
            catch (Exception ex)
            {
    ...
    // Add the missing UpdateGraphEventAsync method to the GraphClient class to resolve the CS1061 error.
    public class GraphClient
    {
        ...
        public GraphClient(string apiKey, string apiSecret, string tenantId)
        {
            _apiKey = apiKey;
        ...
        public async Task<Microsoft.Graph.Event> CreateGraphEventAsync(Microsoft.Graph.Event graphEvent, string userName)
        {
            try
            {
                var client = new Microsoft.Graph.GraphServiceClient(new Microsoft.Graph.DelegateAuthenticationProvider(async (requestMessage) =>
                ...
                var createdEvent = await client.Users[userName].Events.Request().AddAsync(graphEvent);
                return createdEvent;
            }
            catch (Exception ex)
            {
        ...
        // Add the missing UpdateGraphEventAsync method
        public async Task<Microsoft.Graph.Event> UpdateGraphEventAsync(Microsoft.Graph.Event graphEvent, string userName, string eventId)
        {
            try
            {
                var client = new Microsoft.Graph.GraphServiceClient(new Microsoft.Graph.DelegateAuthenticationProvider(async (requestMessage) =>
                ...
                var updatedEvent = await client.Users[userName].Events[eventId].Request().UpdateAsync(graphEvent);
                return updatedEvent;
            }
            catch (Exception ex)
            {
