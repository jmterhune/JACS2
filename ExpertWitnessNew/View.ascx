...
<script type="text/javascript">
    ...
    function PageInit() {
        $(".view-comment").on("click", function (e) {
            ...
            Swal.fire({ title: 'View Comments', html: comment, icon: 'info', confirmButtonText: 'OK' });
        });
        $('.comment').on('click', function (e) {
            var title = $(this).attr("title");
            e.preventDefault();
            Swal.fire({ title: '', html: title, icon: 'info', confirmButtonText: 'OK' });
        });
    }
    function ShowComment(comment) {

    }
    function ewNotify(type, title, html) {
        if (type === 'success' || type === 'info') {
            new Noty({
                ...
                timeout: 4000,
                progressBar: true,
                text: (title ? '<strong>' + title + '</strong><br>' : '') + html
            }).show();
        } else {
            Swal.fire({ title: title, html: html, icon: type, confirmButtonText: 'OK' });
        }
    }
</script>
