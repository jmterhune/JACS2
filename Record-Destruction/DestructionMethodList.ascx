                        ...
                        <ItemTemplate>
                            <tr>
                                <td class="command-item">
                                    <asp:LinkButton ID="cmdEdit" runat="server" CommandName="edit" CausesValidation="false" CommandArgument='<%#DataBinder.Eval(Container.DataItem,"DestructionMethodID").ToString() %>'><i class="fa fa-pencil"></i></asp:LinkButton>
                                <td class="command-item"><%#Eval("DestructionMethodID")%></td>
                                <td><%#Eval("Description")%></td>
                                <td class="command-item">
                                    <asp:LinkButton ID="cmdDelete" CssClass="confirm" runat="server" CausesValidation="false" CommandName="delete" CommandArgument='<%#DataBinder.Eval(Container.DataItem,"DestructionMethodID").ToString() %>'><i class="fa fa-trash"></i></asp:LinkButton>
                                </td>
                            </tr>
                        </ItemTemplate>
                        <FooterTemplate>
...
<script type="text/javascript">
    ...
    function PageInit() {
        ...
        $("#tblDestructionMethod_length").prepend('<button onclick="return ClearForm()" class="btn btn-primary btn-sm me-2" data-bs-toggle="modal" data-bs-target="#EditDestructionMethodModal"><i class="fa fa-plus"></i>&nbsp;Add Destruction Method</button>');
        table.on('draw', function () {
            $(".confirm").dnnConfirm({
                text: 'Are you sure you wish to Delete the selected Destruction Method?',
                yesText: 'Yes',
                noText: 'No',
                title: 'Delete Destruction Method?'
            });
        });
        table.draw();
    }
