                        ...
                        <ItemTemplate>
                            <tr>
                                <td class="command-item">
                                    <asp:LinkButton ID="cmdEdit" runat="server" CssClass="text-primary" CommandName="edit" CausesValidation="false" CommandArgument='<%#DataBinder.Eval(Container.DataItem,"RecordTypeID").ToString() %>'><i class="fas fa-edit"></i></asp:LinkButton>
                                <td class="command-item"><%#Eval("RecordTypeID")%></td>
                                <td><%#Eval("Description")%></td>
                                <td class="command-item">
                                    <asp:LinkButton ID="cmdDelete" CssClass="text-danger confirm" runat="server" CausesValidation="false" CommandName="delete" CommandArgument='<%#DataBinder.Eval(Container.DataItem,"RecordTypeID").ToString() %>'><i class="fas fa-trash"></i></asp:LinkButton>
                                </td>
                            </tr>
                        </ItemTemplate>
                        <FooterTemplate>
...
<script type="text/javascript">
    ...
    function PageInit() {
        ...
        $(".dt-length").prepend('<button onclick="return ClearForm()" class="btn btn-primary btn-sm me-2" data-bs-toggle="modal" data-bs-target="#EditRecordTypeModal"><i class="fa fa-plus"></i>&nbsp;Add Record Type</button>');
        table.on('draw', function () {
            $(".confirm").not('[data-swal-bound]').attr('data-swal-bound', '1').on('click', function (e) {
                e.preventDefault();
                var href = this.href || '';
                Swal.fire({
                    title: 'Delete Record Type?', text: 'Are you sure you wish to Delete the selected Record Type?', icon: 'warning',
